...
def api_register(request):
    ...
    if username and password and email:
        if len(username) < 10 and len(password) < 10:
            ...
            if u=='':
                info='用户名为空'
            elif not u.exists() and not e.exists():
...
def locust(request):
    ...
    min_wait = request.POST.get("min_wait")
    max_wait = request.POST.get("max_wait")
    name = request.POST.get("name")
    ...
    try:
        ...
        print(e)
    return render(request,'locust.html',{"link": "index/locust"})
